...
15. **Default progression rule**: if the user says things like *“next step”*, *“do the next step”*, or *“continue with the next step”*, move to the **next numbered framework step by default**, whether it is optional or mandatory. Do not skip ahead unless the user explicitly says to skip, or `--force` is used.
